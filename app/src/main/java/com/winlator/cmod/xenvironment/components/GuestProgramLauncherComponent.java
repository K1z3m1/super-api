...
public class GuestProgramLauncherComponent extends EnvironmentComponent {
    ...
    private int execGuestProgram() {
        ...
        if (shortcut != null) {
            ...
            // Apply the container-level frame generation settings for direct
            // (non-shortcut) launches too, instead of forcing it off.
            FrameGenManager.ensureRuntimeInstalled(context, container);
            FrameGenManager.writeConfig(container);
            FrameGenManager.applyLaunchEnv(container, execEnvVars);
        }
